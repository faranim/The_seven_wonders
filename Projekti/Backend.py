...
def story_intro():
    ...
    print("---------------------------------------------------------------")
    print("Tavoitteenasi on matkustaa näkemään seitsemän eri nähtävyyttä, mahdollisimman vähäisillä hiilidioksidipäästöillä.")
...
def matkamuistot(maa_id):
    ...
    id = x[0]
    mycursor = yhteys.cursor()
    mycursor.execute("SELECT game_id, goal_id FROM goal_reached WHERE game_id ='"+str(id)+"' and goal_id = '"+str(maa_id)+"'")
    result = mycursor.fetchall()
    if len(result)==0:
...
while peli:
    ...
    if maa == 'EFHK':
        ...
        if tapa == '5':
            print(get_airports())
            maa = input('Valitse mihin kohteeseen haluat lentää ensimmäisenä, yllä olevien ICAO-koodein avulla: '). upper()
    ...
    if maa == 'EFRO':
        ...
        if scenery == True:
            ...
            if tapa == '2':
                scenery = True
            if tapa == '3':
                print("Tapaat joulupukin! Koska olet ollut kiltti, hän vie sinut näkemään revontulia. Palkinnoksi sinulta vähennetään 500kg päästöistäsi")
